...
if st.session_state.page == "인트로":
                ...
                for s in subs:
                    if isinstance(s, dict):
                        if q in s["title"].lower():
                            ...
                            inner_hits = [x for x in s["subs"] if q in x.lower()]
                            if inner_hits:
                                hits.append({"title": s["title"], "subs": inner_hits})
                    elif q in s.lower():
                        hits.append(s)
                if hits: search_results[main] = hits

        if search_results:
            st.markdown("<br><div style='font-weight:700; color:#005bac;'>🔎 검색 결과</div>",
                        unsafe_allow_html=True)
            with st.container():
                    ...
                    for sub in subs:
                        if isinstance(sub, dict):
                            with st.expander(sub["title"]):
                                for s in sub["subs"]:
                                    st.button(s, key=f"search-{s}", use_container_width=True,
    ...
    # 📚 전체 메뉴
    st.markdown("<br><div style='font-weight:700; color:#1f2937;'>📚 전체 목차</div>",
                unsafe_allow_html=True)
    with st.container():
        st.markdown('<div class="big-card">', unsafe_allow_html=True)
        for main, subs in sections.items():
